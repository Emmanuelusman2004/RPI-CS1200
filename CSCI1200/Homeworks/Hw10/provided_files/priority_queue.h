...
class DistancePixel_PriorityQueue {
 ...
 private:
  ...
  void percolate_up(int i) { //push you call percolate up
    //
    // ASSIGNMENT: Implement this function
    //
    DistancePixel* element = m_heap[i];
    while (i > 0) {
        int parent_index = get_parent(i);
        DistancePixel* parent_element = m_heap[parent_index];
        if (*element < *parent_element) {
            // Swap element with parent
            m_heap[i] = parent_element;
            m_heap[parent_index] = element;
            // Update backpointers
            backpointers[element] = parent_index;
            backpointers[parent_element] = i;
            // Update i to continue checking upwards
            i = parent_index;
        } else {
            break; // element is in the right place
        }
    }
}
  
  void percolate_down(int i) { //pop you call percolate 
    //
    // ASSIGNMENT: Implement this function
    //
  while (has_left_child(i)) {
    ...
    if (has_right_child(i) && m_heap[get_right_child(i)] < m_heap[get_left_child(i)])
      child_index = get_right_child(i);
    else
      child_index = get_left_child(i);
    if (m_heap[child_index] < m_heap[i]) {
      // Swap element with child
      DistancePixel* element = m_heap[i];
      m_heap[i] = m_heap[child_index];
      m_heap[child_index] = element;
      // Update backpointers
      backpointers[element] = child_index;
      backpointers[m_heap[i]] = i;
      // Update i to continue checking downwards
      i = child_index;
    } 
    else {
      break; // element is in the right place
