...
-q (specific move): O(log(f))

-q (all): O(log(f) * m) = O(log(f))

-f: O(f^2) (because we call function then use insert)
...
forgive the whitespace error, I hope you dont take too many points off for that. other than that please take it easy on me :(
