...
public:
	EquilateralTriangle(const string &name, const vector<Point> &vertices) : Triangle(name, vertices), IsoscelesTriangle(name, vertices)
	{
		if (!HasAllEqualSides())
			throw -1;
...
class Diamond : public Quadrilateral, public Parallelogram, public Kite
{
private:
	string name;
	vector<Point> vertices;
public:
	Diamond(const string & name, const vector<Point> &vertices) : Quadrilateral(name, vertices), Parallelogram(name, vertices), Kite(name,vertices) {
