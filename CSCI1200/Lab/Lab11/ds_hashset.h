...
private:
  ...
  float occupancylvl = .5;///////////////////////////////////////////////////////////////////////////check how this works
...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    unsigned int store = m_hash(key)%m_table.size();
    hash_list_itr finding = std::find(m_table[store].begin(),m_table[store].end(),key);
    if (finding!=m_table[store].end()) {
      iterator it(this,store,finding);
      return (std::make_pair(it,false));
    }
    else {
      m_table[store].push_back(key);
      m_size++;
      iterator it(this,store,m_table[store].end()--);
  ...
  iterator begin() {
    ...
    iterator p(this);
    hash_list_itr it = m_table[0].begin();
    p.m_list_itr = it;
    p.m_index = 0;
    return p;
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    for (iterator it = begin(); it != end();it++) {
      unsigned int store = m_hash(*it)%new_size;
      new_table[store].push_back(*it);
    }
    m_table = new_table;
    m_size = new_size;
  }
