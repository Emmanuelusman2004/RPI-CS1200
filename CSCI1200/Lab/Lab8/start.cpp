...
bool checker(int x,int y,std::vector<Point> &v) {
...
void print_grid_solution(const std::vector<std::vector<GRID_STATUS> > & blocked_grid, unsigned int start_x, unsigned int start_y, std::vector<Point> &v) { 
    for (unsigned int y=0; y<blocked_grid[0].size(); ++y) {
    for (unsigned int x=0; x<blocked_grid.size(); ++x) {
      if (x == start_x && y == start_y)
        std::cout << " S";
...
int paths(int x, int y, std::vector<std::vector<GRID_STATUS>  > &blocked_grid, std::vector<Point> &v){
    if (blocked_grid[x][y] == GRID_BLOCKED) {
      ...
      if (x == 0) {
        return paths(x,y-1, blocked_grid, v);
        v.push_back(Point(x,y));
      }
      else {
        return paths(x - 1, y, blocked_grid, v);
        v.push_back(Point(x,y));
      }
    }
    else{

        return paths(x-1,y, blocked_grid, v) + paths(x, y-1, blocked_grid, v);
        v.push_back(Point(x,y));
    }
}
...
int main(int argc, char* argv[]) {
  ...
  std::vector<Point> v;
  std::cout<<paths(start_x, start_y, blocked_grid, v)<<std::endl;
  print_grid_solution(blocked_grid, start_x, start_y, v);
